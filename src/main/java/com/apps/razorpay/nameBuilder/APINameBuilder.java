package com.apps.razorpay.nameBuilder;
...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "Wallet_Payment".equals(entityTypeName) ? "Wallet_Payments" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Upi_Payment".equals(entityTypeName) ? "Upi_Payments" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Subscription".equals(entityTypeName) ? "Subscriptions" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Card_Payment".equals(entityTypeName) ? "Card_Payments" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Netbanking_Payment".equals(entityTypeName) ? "Netbanking_Payments" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
